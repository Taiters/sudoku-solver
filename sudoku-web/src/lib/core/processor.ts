...
export class SudokuFrameProcessor {
    ...
    constructor(cv: typeof openCV, width: number, height: number, options: Partial<SudokuFrameProcessorOptions> = {}) {
        this.cv = cv;
        this.options = {
            ...DEFAULT_OPTIONS,
            ...options,
        ...
        this.target = cv.matFromArray(4, 1, cv.CV_32FC2, [
            0, 0,
            this.options.sudokuRegionSize, 0,
            0, this.options.sudokuRegionSize,
            this.options.sudokuRegionSize, this.options.sudokuRegionSize,
        ]);
    ...
    processFrame(frame: Uint8Array): SudokuFrameData | null {
        ...
        this.cv.threshold(this.binary, this.binary, 0, 255, this.cv.THRESH_BINARY_INV + this.cv.THRESH_OTSU);

        // Get the quads
        this.cv.findContours(this.binary, this.contours, this.hierarchy, this.cv.RETR_TREE, this.cv.CHAIN_APPROX_SIMPLE);
        for (let i = 0; i < this.contours.size(); i++) {
            const cnt = this.contours.get(i);
            const poly = this.getMat();
            this.cv.approxPolyDP(cnt, poly, this.options.arcLengthThreshold * this.cv.arcLength(cnt, true), true);
        ...
        const resultSize = new this.cv.Size(this.options.sudokuRegionSize, this.options.sudokuRegionSize);
        const source = this.addToMatBag(() => this.cv.matFromArray(4, 1, this.cv.CV_32FC2, corners.flat()));
        const transform = this.addToMatBag(() => this.cv.getPerspectiveTransform(
            source,
            this.target,
        ));
        ...
        for (let i = 0; i < this.lines.rows; i++) {
            const startPoint = new this.cv.Point(this.lines.data32S[i * 4], this.lines.data32S[i * 4 + 1]);
            const endPoint = new this.cv.Point(this.lines.data32S[i * 4 + 2], this.lines.data32S[i * 4 + 3]);
